...
-- NOTE: Changed the script so it load files on user input, instead of on opening new files
...
function open_files_on_command()
    o.disabled = false
    find_and_add_entries()
end

mp.add_key_binding(nil, "load-files", function() open_files_on_command() end, {repeatable=false})