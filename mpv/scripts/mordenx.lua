...
local user_opts = {
    ...
    showWcTitle = false,        -- show title on the Window Control bar
    wcAlpha = 255,              -- alpha of the window control bar
    ...
    seekbarhandlesize = 1,    -- size ratio of the knob handle
...
local osc_styles = {
    ...
    vidTitle = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs" .. user_opts.titleFontSize .. "\\q2\\fn" .. user_opts.titlefont .. "}",
            ...
            if not (element.slider.markerF == nil) and (slider_lo.gap > 0) then
                ...
                for _,marker in pairs(markers) do
                    if (marker >= element.slider.min.value) and
                        ...
                        -- Just draw a line instead of a triangle like in the default config
                        --top
                        if (slider_lo.nibbles_top) then
                            static_ass:rect_cw(s - 1, slider_lo.gap/3, s + 1, slider_lo.gap);
                        end
                        --bottom
                        if (slider_lo.nibbles_bottom) then
                            static_ass:rect_cw(s - 1, elem_geo.h-slider_lo.gap, s + 1, elem_geo.h-slider_lo.gap/3);
                        end
                    end
                end
            end
...
function window_controls()
    ...
    -- Window Title
    if (user_opts.showWcTitle == true) then
